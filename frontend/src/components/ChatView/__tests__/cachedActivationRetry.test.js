...
test('cached activation retries only transient failures and stops after three attempts', () => {
  ...
  assert.equal(cachedActivationRetryDelay(new TypeError('NetworkError when attempting to fetch resource.'), 0), 750)
  assert.equal(cachedActivationRetryDelay(new TypeError('Cannot read properties of undefined'), 0), null)
