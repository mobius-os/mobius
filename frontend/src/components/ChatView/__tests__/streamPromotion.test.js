...
test('a durable generated file survives a sparse live replay', () => {
  const durableFile = {
  ...
  const carried = carryDurableBlockState(
    ...
    [{
      type: 'generated_files', files: [durableFile],
    }],
  )
...
test('final promotion cannot erase a generated-file card from its durable row', () => {
  ...
  const messages = [{
    ...
    role: 'assistant',
    content: 'Creating it.',
    blocks: [{ type: 'generated_files', files: [durableFile] }],
  }]
  ...
  assert.deepEqual(promoted.blocks.at(-1), {
    type: 'generated_files', files: [durableFile],
  })
})
