...
export default function SharedApp() {
  ...
  useEffect(() => {
    if (!instanceQuery.data?.entry_path || phase !== 'workspace') return undefined
    ...
    ;(async () => {
      try {
        ...
        const response = await api.sharedApps.output(instanceId, entry, { signal: controller.signal })
        if (!response.ok) throw new Error(`The shared build could not be loaded (${response.status}).`)
        const html = await response.text()
        const loadText = async path => {
          const dep = await api.sharedApps.output(instanceId, path, { signal: controller.signal })
  ...
  }, [instanceId, instanceQuery.data?.entry_path, instanceQuery.data?.updated_at, phase])
