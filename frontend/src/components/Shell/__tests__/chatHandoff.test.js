...
test('direct chat actions hand focus to the destination composer', () => {
  ...
  assert.match(startUserNewChatPresentation,
    /flushSync\(\(\) => \{[\s\S]*setNewChatPresentation\(presentation\)[\s\S]*navTo\('chat', \{ chatId, paneId: ws\.focusedPaneId \}\)[\s\S]*requestComposer\(chatId/,
    'the client id must enter workspace and history before focus, independent of allocation')
  assert.match(shell,
