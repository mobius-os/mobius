...
// src/runtime/network.js
var READ_TIMEOUT_MS = 2500;
function fetchBounded(url, init2) {
  const ctrl = typeof AbortController !== "undefined" ? new AbortController() : null;
  const opts = ctrl ? { ...init2, signal: ctrl.signal } : init2;
  let timer;
...
async function fetchWithAppToken(getToken, url, init2 = {}, fetcher = fetch) {
  const run = async (token2) => {
    if (!token2) throw new Error("mobius: app token unavailable");
    return fetcher(url, {
      ...init2,
      headers: { ...init2.headers || {}, Authorization: `Bearer ${token2}` }
    });
  };
...
var DB_NAME = "mobius-outbox";
var SIGNAL_DB_NAME = "mobius-signals";
var STORE = "ops";
var SIGNAL_STORE = "signals";
var CACHE_STORE = "cache";
var OUTCOME_STORE = "write_outcomes";
var DB_VERSION = 3;
var SIGNAL_DB_VERSION = 1;
var MAX_WRITE_OUTCOMES = 200;
var MAX_PENDING_SIGNALS = 500;
var MAX_PENDING_SIGNAL_BYTES = 2 * 1024 * 1024;
var MAX_GLOBAL_PENDING_SIGNALS = 2e3;
var MAX_GLOBAL_PENDING_SIGNAL_BYTES = 8 * 1024 * 1024;
var SIGNAL_SEND_BATCH = 100;
...
function openDb() {
  return new Promise((resolve, reject) => {
    ...
    req.onblocked = () => {
      if (!settled) {
        settled = true;
        reject(new Error("mobius-outbox open blocked"));
      }
    };
  });
}
function openSignalDb() {
  return new Promise((resolve, reject) => {
    ...
    req.onblocked = () => {
      if (!settled) {
        settled = true;
        reject(new Error("mobius-signals open blocked"));
      }
    };
  });
}
...
function makeStorage({ appId, appInstanceId = null, getToken, isOnline = null }) {
  ...
  function cacheConfirmVersion(path, expectedVer, serverVersion) {
    ...
    return withStore(CACHE_STORE, "readwrite", (store, box) => {
      box.value = false;
      const get2 = store.get(cacheKey(path));
      get2.onsuccess = () => {
        const current = get2.result;
  ...
  function assertReadKind(path, storedKind, wantKind) {
    ...
    if (stored !== wantKind) {
      throw new Error(
        `mobius.storage: ${path} holds ${stored}; read it with ` + (stored === "json" ? "get()" : stored === "text" ? "getText()" : "getBlob()")
      );
    }
  }
  ...
  function notify(path, data) {
    const set2 = subscribers.get(path);
    if (!set2) return;
  ...
  async function send(op) {
    ...
    const init2 = { method: op.method, headers: {} };
    if (op.method === "PUT") {
      if (op.ifMatch) init2.headers["If-Match"] = op.ifMatch;
      if (op.ifNoneMatch) init2.headers["If-None-Match"] = "*";
      if (op.kind === "blob" || op.kind === "text") {
        init2.headers["Content-Type"] = op.contentType || (op.kind === "blob" ? "application/octet-stream" : "text/plain;charset=utf-8");
        init2.body = op.data;
      } else {
        init2.headers["Content-Type"] = "application/json";
        init2.body = JSON.stringify(op.data);
      }
    }
    const res = await fetchWithAppToken(getToken, url, init2);
    ...
    err.fatal = res.status >= 400 && res.status < 500 && ![401, 408, 429].includes(res.status) && !err.conflict;
    throw err;
  }
  ...
  async function setBlob(path, blob, opts) {
    ...
    if (blob.size > MAX_BLOB_BYTES) {
      throw new Error(
        `mobius.storage.setBlob: ${path} is ${blob.size} bytes, over the ${MAX_BLOB_BYTES}-byte limit (use OPFS or a direct upload for large media)`
      );
    }
  ...
  function scheduleSignalRetry(response = null) {
    ...
    const backoffDelay = Math.min(Math.max(_signalRetryDelay, 1e3), 5 * 60 * 1e3);
    delay = Math.min(Math.max(delay, backoffDelay), 24 * 60 * 60 * 1e3);
    _signalRetryDelay = Math.min(backoffDelay * 2, 5 * 60 * 1e3);
  ...
  function drainSignals(force = false) {
    ...
    if (navigator.locks && navigator.locks.request) {
      return navigator.locks.request(`mobius-signals-${appId}`, drainSignalsInner).catch(() => {
      });
    }
  ...
  async function fetchValueWithVersion(path, kind = "json", wantVersion = false) {
    ...
    const version = res.headers && typeof res.headers.get === "function" ? res.headers.get("ETag") || res.headers.get("etag") || void 0 : void 0;
    if (res.status === 404) return { value: null, version: void 0 };
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    let value;
  ...
  function subscribeWith(path, cb, getter, kind = "json") {
    ...
    let set2 = subscribers.get(path);
    if (!set2) {
      set2 = /* @__PURE__ */ new Set();
      subscribers.set(path, set2);
    }
    let delivered = false;
    const wrapped = (v) => {
      delivered = true;
...
function createUseDocument(storage, reactProvider = null) {
  return function useDocument(path, opts = {}) {
    ...
    const controller = React.useMemo(() => {
      const initialValue2 = typeof initialOpt === "function" ? initialOpt() : initialOpt;
      return {
        path,
        enabled,
    ...
    const setValue = React.useCallback((owner, value, status = "ready", lastError = null) => {
      owner.value = value;
      setState((previous) => previous.controller === owner ? { controller: owner, value, status, lastError } : previous);
    }, []);
    const refresh = React.useCallback(async () => {
      if (!enabled) return controller.value;
      try {
        const loaded = storage.getWithVersion ? await storage.getWithVersion(path, "json") : { value: await storage.get(path), version: void 0 };
        ...
        setState((previous) => previous.controller === controller ? { controller, value: controller.value, status: "error", lastError: e } : previous);
    ...
    React.useEffect(() => {
      ...
      setState((previous) => previous.controller === controller && previous.status === status && previous.lastError == null ? previous : { controller, value: controller.value, status, lastError: null });
    ...
    const update = React.useCallback((fn) => {
      ...
      const run = async () => {
        ...
        for (; ; ) {
          ...
          const merged = customMerge ? customMerge(base, mine, theirs == null ? initialValue : theirs) : defaultDocumentMerge(base, mine, theirs == null ? initialValue : theirs, identity);
          ...
          try {
            ...
            setState((previous2) => previous2.controller === controller ? { controller, value: controller.value, status: "error", lastError: e } : previous2);
            if (typeof onError === "function") {
    ...
    const visibleState = state.controller === controller ? state : { value: controller.value, status: enabled ? "loading" : "idle", lastError: null };
    return {
...
var SIGNAL_BUF_CAP = 500;
var SIGNAL_BATCH_CAP = 100;
var SIGNAL_FLUSH_INTERVAL_MS = 5e3;
var SIGNAL_NAME_MAX = 80;
var SIGNAL_PAYLOAD_KEYS_MAX = 20;
var SIGNAL_PAYLOAD_KEY_MAX = 80;
var SIGNAL_PAYLOAD_STRING_MAX = 500;
var SIGNAL_EVENT_BYTES_MAX = 4e3;
function _signalServerBytes(value) {
  const json = JSON.stringify(value);
  let bytes = 0;
  for (let index = 0; index < json.length; index += 1) {
    bytes += json.charCodeAt(index) <= 127 ? 1 : 6;
  }
  return bytes;
}
...
var EMBED_GUIDANCE = EMBED_NS + "guidance";
var EMBED_READY = EMBED_NS + "ready";
var EMBED_MESSAGE_SENT = EMBED_NS + "message-sent";
var EMBED_TURN_DONE = EMBED_NS + "turn-done";
var EMBED_ERROR = EMBED_NS + "error";
var EMBED_AUTH_EXPIRING = EMBED_NS + "auth-expiring";
var EMBED_BOOTSTRAP_READY = EMBED_NS + "bootstrap-ready";
var EMBED_CONTEXT_REQUEST = EMBED_NS + "context-request";
var EMBED_CONTEXT_RESPONSE = EMBED_NS + "context-response";
var EMBED_GUIDANCE_MAX_LENGTH = 300;
...
var EMBED_STICKY = /* @__PURE__ */ new Set(["ready", "error"]);
...
function makeEmbedAuthorizationHandoff({
  mint,
  post,
  onAttemptError = () => {
  },
  ...
  retryDelays = [250, 1e3, 3e3, 5e3],
  acknowledgementTimeoutMs = 5e3,
  makeAuthorizationId = () => typeof crypto !== "undefined" && typeof crypto.randomUUID === "function" ? crypto.randomUUID() : `authorization-${Date.now()}-${Math.random().toString(36).slice(2)}`
}) {
  let destroyed = false;
  let pending = null;
  let retryTimer = null;
...
var EMBED_BOOTSTRAP_TIMEOUT_MS = 15e3;
function makeChat({ appId, getToken, storage }) {
  async function appChatFetch(url, init2 = {}) {
    return fetchWithAppToken(getToken, url, init2);
  }
  ...
  const chat = async function chat2(opts = {}) {
    ...
    function chatOptionLabel(chat3) {
      const label = chat3 && typeof chat3.scope_label === "string" ? chat3.scope_label.trim() : "";
      const title = chat3 && typeof chat3.title === "string" ? chat3.title.trim() : "";
      if (label) return label;
      if (title) return title;
      return chat3 && chat3.id ? `Chat ${String(chat3.id).slice(0, 8)}` : "Chat";
    }
    function renderChatOptions(chats) {
      if (!selectEl) return;
      const options = [];
      const seen = /* @__PURE__ */ new Set();
      for (const chat3 of chats || []) {
        if (!chat3 || !chat3.id) continue;
        const id = String(chat3.id);
        if (seen.has(id)) continue;
        seen.add(id);
        options.push({ ...chat3, id });
      }
      ...
      selectEl.replaceChildren(...options.map((chat3) => {
        const option = document.createElement("option");
        option.value = chat3.id;
        option.textContent = chatOptionLabel(chat3);
        return option;
      }));
...
var SPLIT_WIDE_BP = 600;
var SPLIT_FLICK_VEL = 0.4;
var SPLIT_DEAD_ZONE = 24;
var SPLIT_ARROW_STEP = 0.04;
var SPLIT_STATES = { PILL: "pill", SPLIT: "split", FULL: "full" };
...
function makeSplit() {
  return function split(opts = {}) {
    ...
    function persist() {
      ...
      try {
        sessionStorage.setItem(persistKey, JSON.stringify({ ratio, state }));
      } catch (e) {
      }
    }
    function isWide() {
    ...
    function applyState() {
      ...
      if (contentEl) {
        contentEl.style.pointerEvents = state === SPLIT_STATES.FULL ? "none" : "auto";
      }
    }
...
function makeNav() {
  ...
  function open(label, onBackOrHandlers, onForwardArg) {
    ...
    const handlers = onBackOrHandlers && typeof onBackOrHandlers === "object" ? onBackOrHandlers : { onBack: onBackOrHandlers, onForward: onForwardArg };
...
var _onlineListeners = /* @__PURE__ */ new Set();
...
var runtimeFeatures = Object.freeze({
  idleDocument: true
});
