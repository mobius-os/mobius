...
| Add/install a skill | Ecosystem installs go through `POST /api/skills/install` (`routes/skills.py`; the Skills app + `finding-skills.md` seed drive it); platform skills live in `backend/scripts/seed-skills/` and advance at server start from their recorded `.seed-skills.json` baseline, while local changes stay in place for review; the index (`skills-index.md`) is generated — never hand-edit it |
...
`backend/scripts/seed-skills/`, so installed skills follow the running source and
a template-only release needs a restart rather than a new container image. An
image built before this handoff still reconciles from its entrypoint with its
baked seeds; the server step stays off until that container is replaced, so the
two never run together.
`.seed-skills.json` records the last platform bytes applied per skill.
An unchanged live copy advances to the next seed. An edited copy stays
...
sidecar. After comparing exact local and platform bytes, the owner can
keep the local text (`init_skills.py --resolve NAME.md --decision keep-local
--expected-sha256 DIGEST`), take the platform version after archiving the local
...
blob reachable from the applied checkout's `HEAD`; if that evidence is absent,
startup preserves the live file and marks it for review.
...
and are installed again if a later running source ships them (for example after
an older image or the baked fallback retired them).
...
Streaming state lives in its own `chat_live_assistants` row, exposed on a Chat instance as `live_assistant`: `PersistTranscript`/`PersistError` replace only that bounded snapshot. A separate column on `chats` is insufficient: SQLite rewrites the overflow payload of a row even when only a small column changes. The snapshot writer does not update the historical row unless the active assistant identity actually changes. Read routes overlay that current assistant on immutable history. `QuestionCommit` merges the card into history before broadcast, `Finalize` performs the terminal merge and clears the live value, and startup reconciliation performs the same merge after a crash. This keeps one-second crash-resilient snapshots without quadratic transcript rewrites as chats grow. The live table has one primary-key row per chat, cleared in the same transaction as finalization/recovery and reclaimed with the chat's existing retention lifecycle. Migration `0045_chat_live_assistants` copies prior snapshots in SQL and drops the old column; it does not decode whole histories in Python, change save cadence, or introduce a concurrency cap.
...
Every mini-app ships a `mobius.json`; the dependency-free source of truth is `backend/app/manifest_contract.py`, used by Store install, explicit local apply, and `backend/scripts/validate-app.py`. Five required non-empty string fields are `id`, `name`, `version`, `description`, and `entry`; `entry` must be the canonical `index.jsx` used by the source/apply lifecycle. The `id` is the package's current slug request (source dir `/data/apps/<slug>/`; `allocate_unique_slug` can diverge it on a collision, and cron registration keys off the resolved `app.slug`), so it uses charset `a-z 0-9 - _`, cannot start with `-`/`_`, and cannot be purely numeric (bare integers are reserved for the numeric-id storage tree).
