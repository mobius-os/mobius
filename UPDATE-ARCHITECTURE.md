...
### Delivery rule: prefer a smaller safe slice

Do not ship a partial served-generation or automatic-rollback mechanism merely
because its state record and unit tests look complete. The first implementation
review found that its import probe changed the supposedly immutable artifact,
restart surfaces did not share one publication owner, image compatibility was
inverted in one transition, and a fixed boot timer could interrupt migrations.
That implementation was removed rather than extended with more states.

The safe foundation may land independently: boot-critical readiness, restart
wait reconciliation, per-app schedule isolation, clear in-app recovery,
actionable image-preservation blockers, and a self-hosted replacement path that
...
- backend and frontend stay on one generation for the whole boot;
- an image-required candidate cannot run on the old image;
- the previous source is proven compatible with the migrated data and runtime;
- rollback is bounded, visible, and a later fixed image can leave recovery;
- protected bootstrap locks cannot follow owner-controlled symlinks; and
- crash tests prove files, manifests, and the selected pointer are durable in
  that order.

Once those gates pass, Python dependency changes require a reviewed image. The
current foundation stops before changing source and hands the incomplete case
to an agent; the existing baked fallback remains the last-resort recovery
boundary. This is less automatic, but substantially simpler and safer than
pretending a shared interpreter or mutable source copy is reversible.

The foundation intentionally stops a Python-dependency release before source
mutation because its current replacement executors cannot yet validate the new
source and new interpreter together. That temporary path still offers an agent
handoff; it is a documented gap from the no-agent target below, not a claim
that predictable image activation is complete.
...
| `dependency_sync` (legacy client value; no longer emitted by the backend) | Image replacement until generation-specific dependency environments exist |
...
last, not a mandatory shared Git-history shape. Synthetic baselines remain only
a finite adoption bridge for older installs, not a second steady-state package
model.
...
0. Land the independent safety foundation described at the top of this file:
   boot-critical readiness, immediate restart-wait reconciliation, per-app
   schedule isolation, clear fallback repair, preservation blockers, and the
   host-owned self-hosted replacement path. This step does not claim frozen
   publication or automatic source rollback.
...
4. Replace platform per-commit replay and its predictor/equivalence path with
   ...
   This slice removes only the platform caller, predictor, and platform replay
   state; it does not replace the app updater's separate merge/resolver flow.
...
7. Keep the app updater's existing merge/resolver model, while tightening its
...
2. **Unsafe source was allowed to restart.** Merged PR `#1295` added a
   ...
   completion to a recent ready boot. Its ready receipt is still time-bounded
   rather than fully generation-bound; the target contract above closes that
   remaining gap.
