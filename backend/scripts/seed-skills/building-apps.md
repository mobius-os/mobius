...
## UI conventions stay in the base workflow

This advanced extension is always read alongside
`building-apps-quickstart.md`, which owns scoped stylesheets, theme tokens,
accessibility, touch targets, reduced motion, empty states, and the ban on
native dialogs. When the brief needs a fixed app shell, sheet, tabs, chat, or
split pane, read only the matching structure in `app-component-shapes.md`.

Do not copy the component catalog into this advanced runtime guide. Packaging,
storage, services, device access, agents, and navigation change how an app
runs; they do not create a second visual contract.
