...
**Carry forward an unanswered question — never silently retire it.** A question card from an earlier brief that went *untapped* is NOT answered: no tap ≠ "no" — the partner may simply not have engaged that night (the feedback surface is easy to scroll past). If the decision is still genuinely useful and unresolved, **re-include the SAME question in tonight's brief** rather than dropping it — only retire it once the partner actually answers it (it appears in `prev-question-answers.json`), the decision goes moot, or they resolve it in a discuss-this-brief chat. Carry the same card forward (don't spawn variants); a short "still open from <date>" keeps it honest without nagging. A genuinely-useful question must survive until the partner sees it on a night they engage — that is how the loop closes.
