...
**Step 2 (consolidation) is the non-deferrable floor — do it first and commit it** (`pm-commit 'memory: consolidated chat notes'`), before the read-trace diff (step 1) or the broader reorg (steps 3–6). Those are the *deep* work: valuable on a quiet night, the first to cut on a busy one. If you do only one Memory thing tonight, it's this.
