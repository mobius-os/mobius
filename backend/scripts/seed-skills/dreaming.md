...
### 0. REVIEW YOUR OWN RECENT RUNS — cheap, first, 1–2 turns

Before anything else, read `inputs/dreaming-run-history.txt`. It is your own track record: the last ~14 nights' exit codes (with durations), recent WARN/ERROR/turn-steering lines from your `dreaming.log`, and your last 10 edits to this skill. Ask one question: **is there a RECURRING failure or friction across the last several nights** — repeated `exit=2` max_turns deaths, the same phase always over budget, a near-timeout creeping up, a step that keeps producing nothing? A pattern across ≥2 nights is a signal to ACT ON tonight — you carry it into phase 2's skill edit. This is one read + one decision; don't let it grow into a phase of its own (the brief is still the floor). If the file says it's a first tracked run, note it and move on.
...
- **Act on your OWN run-history (`inputs/dreaming-run-history.txt`), not just the interviews.** It is your real track record: per-night exit codes (+ durations), recent `dreaming.log` friction, and your last 10 edits to this skill. If phase 0 surfaced a RECURRING pattern across ≥2 nights — repeated `exit=2` (max_turns) nights that shipped only a fallback brief, a phase always over budget, a creeping near-timeout — that is tonight's first skill fix. When the root cause IS something you can change in this skill, make the smallest edit that addresses it (tighten or reorder a phase budget, drop a low-value step, reach the brief earlier) and `pm-commit 'skill(dreaming): <pattern observed -> rule changed>'`. When it is NOT in your control — the runner's `max_turns`, the inputs the wrapper stages, the wall-clock timeout — do NOT edit code you can't see land: `/app` is re-baked on every container boot, so a daytime edit there is silently wiped. Instead write a concrete one-line proposal into the brief's what-needs-your-input (e.g. "3 of the last 5 nights died at the 60-turn cap mid-phase-4; propose raising max_turns to 80, or splitting app-triage into its own cron"). **Read your last 10 self-edits in the history before editing — don't re-introduce a rule a past night already removed for cause.**
