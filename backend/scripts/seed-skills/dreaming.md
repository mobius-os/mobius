...
- **Keep a skill edit general and de-dated.** When a failure earns a skill edit, write the durable *rule plus the check that proves it* ("verify a claimed shell edit landed: `grep` the diff token, `stat` the mtime"), never the dated story of one night ("on <date>, agent X claimed a fix that…"). The incident itself, if worth keeping, is a Mind note you `[[link]]` (per the routing above) — the skill stays a clean ruleset a future run reads cold. A skill that accretes dated anecdotes gets longer and slower to read every night, which is exactly the noise this phase exists to remove.
