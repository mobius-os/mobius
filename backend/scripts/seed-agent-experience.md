...
`pm-commit 'one-line what and why'` — wraps `git add -A && git commit`
but auto-unstages runtime state (chrome profiles, compiled bundles,
logs, etc.) and refuses commits with >50 staged files. Pass
`--allow-broad 'msg'` after confirming the scope is intentional.
