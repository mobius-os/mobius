...
def test_managed_web_login_accepts_the_matching_local_model_identity(monkeypatch, db):
  import asyncio
  from app.routes import auth as auth_routes
  ...
  async def broker_request(method, route, payload=None):
    calls.append((method, route, payload))
    assert (method, route) == ("GET", "/identity")
    return {"linked": True, "subject": "mobius-subject"}
  ...
  assert response.headers["location"] == "/shell/?mobius_login=1"
  assert calls == [("GET", "/identity", None)]


def test_managed_web_login_rejects_a_different_local_model_identity(monkeypatch, db):
  import asyncio
  from app.routes import auth as auth_routes
  ...
  async def broker_request(method, route, payload=None):
    calls.append((method, route, payload))
    assert (method, route) == ("GET", "/identity")
    return {"linked": True, "subject": "different-subject"}
  ...
  assert response.headers["location"] == "/shell/?mobius_login_error=1"
  assert calls == [("GET", "/identity", None)]
...
def test_providers_models_accepts_app_token(client, auth):
  ...
  from app.providers import DEFAULT_HIDDEN_MODELS, KNOWN_MODELS, invalidate_model_cache
  ...
  for provider_id in ("claude", "codex"):
    assert {m["id"] for m in body[provider_id]} == (
      set(KNOWN_MODELS[provider_id]) - DEFAULT_HIDDEN_MODELS[provider_id]
    )
...
def test_providers_models_returns_known_models_on_missing_creds(
  ...
  tier on Claude rows)."""
  from app.providers import DEFAULT_HIDDEN_MODELS, KNOWN_MODELS, invalidate_model_cache, sync_app_model_providers
  from app.config import get_settings
  ...
  assert set(claude_ids) == set(KNOWN_MODELS["claude"]) - DEFAULT_HIDDEN_MODELS["claude"]
  assert set(codex_ids) == set(KNOWN_MODELS["codex"]) - DEFAULT_HIDDEN_MODELS["codex"]
