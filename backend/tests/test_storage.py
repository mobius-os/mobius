...
@pytest.mark.asyncio
async def test_recompile_uses_accepted_runtime_static_assets(
  client, auth, db, monkeypatch, tmp_path,
):
  """Bundle recovery cannot combine accepted code with draft static bytes."""
  import app.compiler as compiler
  import app.models as models
  from app import app_git
  from unittest.mock import patch
  from urllib.parse import urlparse
  from tests.test_apps_install import (
    JSX,
    _fake_async_client,
  )

  base = "https://raw.githubusercontent.com/x/recompile-static/main/"
  manifest = {
    "id": "recompile-static",
    "name": "Recompile static",
    "version": "1.0.0",
  ...
  work = tmp_path / "recompile-static-work"
  bare = tmp_path / "recompile-static.git"
  subprocess.run(
  ...
  (work / "build").mkdir()
  (work / "build" / "asset.txt").write_bytes(b"accepted static")
  app_git._run(work, "add", "-A")
  app_git._run(work, "commit", "-q", "-m", "accepted package")
  subprocess.run(
  ...
  monkeypatch.setattr(
    "app.install._derive_repo_ref", lambda *_: (bare.as_uri(), "main"),
  )
  with patch(
  ...
  editable_asset = Path(row.source_dir) / "static" / "asset.txt"
  editable_asset.write_text("draft static", encoding="utf-8")
  observed = {}

  async def inspect_compile(_source, *, out_path, source_path):
    observed["asset"] = (
      Path(source_path).parent / "static" / "asset.txt"
    ).read_text(encoding="utf-8")
    Path(out_path).write_bytes(b"compiled accepted generation")
    return str(out_path)

  monkeypatch.setattr(compiler, "compile_jsx", inspect_compile)
  await compiler.recompile_app_bundle(db, row, row.jsx_source)

  assert observed["asset"] == "accepted static"
  assert editable_asset.read_text(encoding="utf-8") == "draft static"
