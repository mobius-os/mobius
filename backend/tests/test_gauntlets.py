...
from app.chat import (
  _should_enable_coordination_tools,
  _should_inject_peer_context,
  discard_starting,
)
...
def test_gauntlet_writers_never_receive_shared_peer_context():
  assert _should_inject_peer_context("chat", "run", None) is True
  assert _should_inject_peer_context("chat", "run", object()) is False
  assert _should_inject_peer_context(None, "run", None) is False


def test_coordination_tools_follow_actual_turn_relevance():
  base = {
    "chat_id": "current",
    "delegated": False,
    "project_id": None,
    "coordination_context": "",
    "alive_chat_ids": {"current"},
  }
  assert _should_enable_coordination_tools(**base) is False
  assert _should_enable_coordination_tools(
    **{**base, "alive_chat_ids": {"current", "peer"}},
  ) is True
  assert _should_enable_coordination_tools(
    **{
      **base,
      "coordination_context": (
        "<agent_coordination>event</agent_coordination>"
      ),
    },
  ) is True
  assert _should_enable_coordination_tools(
    **{**base, "project_id": "project-1"},
  ) is True
  assert _should_enable_coordination_tools(
    **{**base, "delegated": True},
  ) is True
...
def _body(app_id, parent_chat_id, *, run_id="run-one"):
  return {
    ...
    "target": "Skyhold Defense",
    "target_path": "/data/apps/skyhold-defense",
    "references": ["Warcraft III tower defence"],
...
def test_retention_never_purges_a_live_standalone_delegation_graph(
  ...
  db.get(models.ChatRun, root_id).status = "completed"
  db.commit()
  purged = purge_expired_chat_tombstones(db)
