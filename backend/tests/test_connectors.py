...
def test_codex_config_env_indirection_keeps_secrets_out_of_argv():
  db = _FakeDb([
    _row("exa", "https://exa.example/mcp", "Authorization", "sk-9"),
    _row("fire", "https://fire.example/mcp", "X-Api-Key", "fk-1"),
  ])
  overrides, env = core.codex_config(db)
  assert 'mcp_servers.exa.url="https://exa.example/mcp"' in overrides
  assert 'mcp_servers.exa.bearer_token_env_var="MOBIUS_CONNECTOR_EXA"' in overrides
  assert env["MOBIUS_CONNECTOR_EXA"] == "sk-9"
  assert (
    'mcp_servers.fire.env_http_headers={"X-Api-Key" = "MOBIUS_CONNECTOR_FIRE"}'
    in overrides
  )
  assert env["MOBIUS_CONNECTOR_FIRE"] == "fk-1"
  # The secret itself must never appear in any override (argv is visible
  # in process listings).
  assert not any("sk-9" in o or "fk-1" in o for o in overrides)
