...
``POST /apply`` (fetch origin + rebase local edits onto the new version, or open
a resolver chat on conflict), and ``POST /restart`` (owner-confirmed
self-restart, same SIGTERM pattern as the normal Settings restart). The status
route is wrapped so a transient git error can never break the Settings page.
"""
