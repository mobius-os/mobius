...
def _assert_execution_live(db, row, principal):
  ...
  app = db.query(models.App).populate_existing().filter_by(id=row.app_id).first()
  if not active or app is None or app.deleted_at is not None or not app.github_access or app.token_nonce != row.app_nonce:
...
async def start_reviews(app_id: int, body: StartReviews,
  ...
  if principal.chat_id is not None or principal.run_id is not None or principal.delegation_id is not None:
    raise HTTPException(403, "Only the owner or Contribute interface can approve a review selection.")
  _validate_submit_app(app_id, principal, db)
  ...
  async with chat_queue.get_transition_lock(f"review-start:{app_id}:{body.request_id}"):
    ...
    if row is not None:
      ...
      row = models.ContributionReviewRun(id=str(uuid.uuid4()), app_id=app_id,
        ...
        targets_json=targets, outcomes_json={}, chat_id=chat.id,
...
async def report_outcome(app_id: int, run_id: str, body: ReviewOutcome,
  ...
  async with chat_queue.get_transition_lock(f"review-outcome:{run_id}"):
    ...
    try:
      ...
      reviews.save_outcome(db, row, item_key, {"state": "needs_you", "summary": str(exc.detail), "head_sha": target["head_sha"]})
      return {"run": reviews.view(row)}
    ...
    if previous.get("state") in {"merging", "merge_unknown", "queued"}:
      if pull.get("merged"):
      ...
      if blocker:
        ...
        async with _github_connection_transaction():
          ...
          if str(actor.get("id") or "") != row.github_actor_id:
            ...
            reviews.save_outcome(db, row, item_key, {**outcome, "state": "merging"})
            try:
