...
def codex_config(db) -> tuple[list[str], dict[str, str]]:
  """Codex `--config` overrides + env vars for enabled connectors.

  Secrets ride env vars (never argv): Authorization keys use codex's
  bearer_token_env_var; custom headers use env_http_headers, whose inline
  TOML maps header name -> env var name.
  """
  overrides: list[str] = []
  env: dict[str, str] = {}
  for row in _enabled(db):
    try:
      overrides.append(f'mcp_servers.{row.slug}.url="{row.url}"')
      if row.auth_header and row.auth_value_encrypted:
        ...
        if row.auth_header.lower() == "authorization":
          overrides.append(f'mcp_servers.{row.slug}.bearer_token_env_var="{var}"')
        else:
          overrides.append(
            f'mcp_servers.{row.slug}.env_http_headers={{"{row.auth_header}" = "{var}"}}'
          )
    except ConnectorError:
      log.warning("connector %s skipped (secret undecryptable)", row.slug)
