...
class ProviderAvailability(Base):
  ...
  Written ONLY inside the ``chat_writer`` actor (``_park_run`` sets it,
  ``_admit_provider_execution`` clears it) so the single serialized writer owns
  it; read by ``background_agents.resolve_background_provider``.
